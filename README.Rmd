...
Note that the solution to the population moment equation $E[\Psi(O,\beta,\theta)]=0$ will recover the ATE $(=\beta)$ and the outcome confounding bridge function parameters $(=\theta)$. Therefore, estimators of $\beta$ and $\theta$ can be constructed as a solution to the moment equation based on the observed data:
...
The result is similar to that using `ivreg` because $h$ is linear in $D$.
